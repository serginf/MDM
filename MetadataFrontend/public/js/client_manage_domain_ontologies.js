...
function removeOntology(graph) {
    $.ajax({
        url: 'artifacts/DOMAIN_ONTOLOGY/'+encodeURIComponent(graph),
        type:'DELETE'
    }).done(function( data ) {
        location.reload();
    });
            ...
            $('#domainOntologies').find('tbody')
                .append($('<tr>')
                    .append($('<td>')
                        ...
                        .text(theObj.user)
                    ).append($('<td>').append($('<a href="/view_domain_ontology?graph='+encodeURIComponent(theObj.graph)+'">').append($('<span class="glyphicon glyphicon-search"></span>')))
